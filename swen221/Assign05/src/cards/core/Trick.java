...
public class Trick {
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		List<Card> cardsPlayed = getCardsPlayed();
		Card firstCard = (cardsPlayed.isEmpty() ? null : cardsPlayed.get(0));

		if (!p.getHand().contains(c)) {
			throw  new IllegalMove("Played out of hand");
		}

		// Out of turn
		if (!getNextToPlay().equals(p.direction)) {
			throw new IllegalMove("Player " + p.direction + " played out of turn");
		}

		// Check out of suit
		boolean hasSuit =  (firstCard == null || !p.getHand().matches(firstCard.suit()).isEmpty());
		boolean playedSuit = (firstCard == null || c.suit().equals(firstCard.suit()));

        // Have suit and not playing it
        if (hasSuit && !playedSuit) {
            throw new IllegalMove("Played out of suit");
        }
	...
	@Override
	public Trick clone() {
		Trick t = new Trick(lead, trumps);
		t.cards = new Card[4];
        int i = 0;
		for (Card c : this.cards) {
			t.cards[i++] = ( c != null ? c.clone() : null);
		}
		return t;
	}
}
