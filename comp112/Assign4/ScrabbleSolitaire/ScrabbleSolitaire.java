...
public class ScrabbleSolitaire implements UIMouseListener, UIButtonListener,  UISliderListener {
    ...
    public ScrabbleSolitaire(){
        ...
        UI.addButton("Replay", this);
        UI.addButton("Save", this);
        UI.addSlider("Playback Delay (ms)", 100, 1000, this);
        this.restart();
    }

    public void sliderPerformed(String action , double amount)
    {
        delay = amount;
    }
    ...
    public void mousePerformed(String action, double x, double y){
        ...
        if (action.equals("pressed"))
        {
            ...
            if (indexb[0] != -1)
            {
                ...
                if (this.hand != null)
                {
                    Tile backup = this.board.pickup(indexb[0], indexb[1]);
                    this.board.place(this.hand , indexb[0], indexb[1]);
                    this.rack.place(backup, 0);
                    this.hand = null;
        ...
        if (action.equals("released") && this.hand != null)
        {
            int index = rack.index(x, y);
            UI.println("Rack Index : " + index);
    ...
    public void buttonPerformed(String button){
        ...
        } else if (button.equals("Replay"))
        {
            ...
            if (file != null)
            {
                Load.PlayBack( file , this , delay );
                this.draw();
            }
        } else if (button.equals("Load"))
        {
            UI.println("Game is already passivily saved by the Game Logger");
        }
     ...
     */
    public void restart(){
        this.restart((int)(Math.random() * Integer.MAX_VALUE ));
    }
    ...
    public void restart(int seed){
        ...
        this.logger = new Logger( "scrabble" + System.currentTimeMillis() + seed + ".log" , seed );
        this.logger.Seed(seed);
